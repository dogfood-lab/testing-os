...
<!-- version:start -->
**v1.22.0** — version actuelle. Consultez [CHANGELOG.md](CHANGELOG.md) pour connaître les nouveautés.
<!-- version:end -->
...
- **Un contrat de politique que vous contrôlez.** Déclarez ce qui compte comme « vérifié » en YAML — un DSL prédicatif limité et sans évaluation (`field`/`op`/`value` + `all`/`any`/`not`/`implies`) — et appliquez-le à tous vos dépôts. Validez une politique avant de la déployer avec `dogfood-verify lint`.
...
| `@dogfood-lab/ingest` | JS | Colle des pipelines : distribution → vérification → persistance → indexation. |
...
| `@dogfood-lab/atlas` | JS | Lit un dépôt et écrit la page qui explique son fonctionnement (`atlas/README.md`) ; `atlas check` contrôle le mappage dans CI. Aucune dépendance entre les éléments ; s’exécute dans n’importe quel dépôt. |
