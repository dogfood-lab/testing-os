...
<!-- version:start -->
**v1.13.0** — version actuelle. Consultez [CHANGELOG.md](CHANGELOG.md) pour connaître les nouveautés.
<!-- version:end -->
...
- **Une page qui explique le fonctionnement d’un dépôt.** Atlas lit les flux de travail, les importations, les écritures et l’historique d’un dépôt, et écrit `atlas/README.md` : ce qui entre, ce qui s’exécute, où cela atterrit, qui le lit, ce qui provoque des erreurs, et où commencer. Aucune phrase n’est écrite par une personne, et `atlas check` fait échouer la CI lorsque la carte cesse de correspondre au code.
...
**Les transitions d’état des résultats sont des preuves et sont en append-only.** Les verbes de fermeture du plan de contrôle du groupe d’agents (`swarm reopen`, `swarm close`) nécessitent une raison et une preuve explicites, et — pour les fermetures d’opérateur — un mode de vérification déclaré ; chaque transition écrit une ligne `finding_events` immuable enregistrant l’autorité qui agit. Aucun chemin automatisé ne peut fermer un résultat en raison d’un manque d’activité ou en rouvrir un par prédiction, et aucun verbe ne peut réécrire l’historique des événements — une information d’identification mal utilisée peut ajouter des transitions, mais chaque ajout est lui-même enregistré.

**Surface du réseau.** Par défaut, la seule sortie est `api.github.com` (en lecture seule : confirmation de la provenance + récupération de la définition du scénario mentionnée ci-dessus). Les deux exceptions sont toutes deux activées par l’utilisateur et décrites ci-dessus : une soumission du fournisseur GitLab (`gitlab.com/api`) et une exécution d’un point d’ancrage XRPL activée par l’opérateur. **Aucune télémétrie, aucune analyse : ce code ne communique jamais avec l’extérieur ; en l’absence de ces deux options, il n’expose aucune surface de réseau au-delà de GitHub.** Le flux de travail du récepteur s’exécute avec `contents: write`, limité à ce dépôt uniquement.
...
| `@dogfood-lab/atlas` | JS | Lit un dépôt et écrit la page qui explique son fonctionnement (`atlas/README.md`) ; `atlas check` contrôle le mappage dans l’intégration continue. Aucune dépendance entre les différents éléments ; s’exécute dans n’importe quel dépôt. |

Outils de test indépendants qui **restent indépendants** mais s’intègrent via des API publiées : [`shipcheck`](https://github.com/mcp-tool-shop-org/shipcheck), [`repo-knowledge`](https://github.com/mcp-tool-shop-org/repo-knowledge), [`ai-eyes-mcp`](https://github.com/mcp-tool-shop-org/ai-eyes-mcp), [`taste-engine`](https://github.com/mcp-tool-shop-org/taste-engine), [`style-dataset-lab`](https://github.com/mcp-tool-shop-org/style-dataset-lab).
...
Nécessite Node ≥ 22. La matrice d’intégration continue exécute Node 22 + 24 sur `ubuntu-latest` ; validée localement sur Node 25.
