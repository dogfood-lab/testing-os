...
18 parts, mostly JavaScript (175 files). Work enters through 8 doors; the busiest is registry-ci, which reaches 3 parts and commits into the repository (pages-ci reaches 5 but commits nothing). It publishes to npm and a container image.
...
- pages/out/.nojekyll is now written by pages/build-pages.mjs.
...
**pages-ci** runs pages/build-metrics.mjs, pages/build-pages.mjs, pages/build-stats.mjs and 4 more, reaches anchor, ledger and verifiers, writes to pages/, registry/badges/, registry/metrics.json, registry/snippets/, registry/timeline.json and site/src/, and deploys the site.

**anchor-xrpl** runs anchor/xrpl/scripts/compute-root.mjs, anchor/xrpl/scripts/emit-anchor-event.mjs and registry/scripts/build-anchors.mjs, reaches verifiers, writes to anchor/xrpl/manifests/, anchor/xrpl/partition-root.json, ledger/events/events.jsonl and registry/anchors.json, commits anchor/xrpl/manifests/*.json, ledger/events/events.jsonl and registry/anchors.json, then pushes, opens an issue when it fails, and opens a pull request.
...
- **registry/metrics.json** is written by pages/build-metrics.mjs and read by nothing else in this repository.
...
- **registry/timeline.json** is written by pages/build-timeline.mjs and read by nothing else in this repository.
...
- **anchor/xrpl/anchor-result.json** is written by anchor/xrpl/scripts/post-anchor.mjs.
- **anchor/xrpl/manifests/** is written by .github/workflows/anchor-xrpl.yml and anchor/xrpl/scripts/compute-root.mjs.
- **anchor/xrpl/partition-root.json** is written by anchor/xrpl/scripts/compute-root.mjs.
- **ledger/events/** is written by .github/workflows/anchor-xrpl.yml, .github/workflows/attestor-ci.yml, attestor/scripts/emit-key-event.mjs and packages/repomesh-cli/src/key/rotate-revoke.mjs.
...
- **registry/metrics.json** is written by pages/build-metrics.mjs.
...
- **registry/timeline.json** is written by pages/build-timeline.mjs.
