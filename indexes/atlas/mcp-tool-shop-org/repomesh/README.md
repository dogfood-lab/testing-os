...
18 parts, mostly JavaScript (175 files). Work enters through 9 doors; the busiest is pages-ci, which reaches 6 parts. It publishes @mcptoolshop/repomesh (packages/repomesh-cli) to npm and a container image. People run repomesh.
...
4. It writes to registry/anchors.json, registry/badges/, registry/snippets/ and registry/trust.json.
5. It deploys the site.

## Who reads the results

- **registry/** is read by packages/repomesh-cli/src/verify/verify-all.mjs, pages (5 files) and scripts/verify.sh (found by text).

## The other doors

**anchor-xrpl** runs anchor/xrpl/scripts/compute-root.mjs, anchor/xrpl/scripts/emit-anchor-event.mjs, packages/repomesh-cli/scripts/build.mjs and 1 more, checks anchor/xrpl/config.json, anchor/xrpl/package-lock.json, anchor/xrpl/package.json and 4 more, reaches verifiers, writes to anchor/xrpl/manifests/, ledger/events/events.jsonl, registry/anchors.json and registry/trust.json, commits anchor/xrpl/manifests/*.json, ledger/events/events.jsonl and registry/anchors.json, then pushes to a branch for review, never to main, opens an issue when it fails, and opens a pull request.

**Release** runs packages/repomesh-cli/scripts/build.mjs and packages/repomesh-cli/tests/, checks anchor/xrpl/config.json, anchor/xrpl/package-lock.json, anchor/xrpl/package.json and 4 more, reaches verifiers, and publishes @mcptoolshop/repomesh (packages/repomesh-cli) to npm and a container image.
...
- **registry** is run as a child process by 1 part (tools) and sits on the path of 3 doors.
- **pages** is run as a child process by 1 part (tools) and sits on the path of 1 door.
- **ledger/events/events.jsonl** is written by .github and read by .github, anchor, attestor, ledger, pages, policy, registry, repomesh-cli and tools; a hand edit reaches every reader.
- **ledger/nodes/** is written by tools and read by attestor, ledger, policy, registry and tools; a hand edit reaches every reader.
...
- **ledger/nodes/** is written by tools/join-node.mjs.
...
- **ledger/events/events.jsonl** is written by .github/workflows/anchor-xrpl.yml and .github/workflows/attestor-ci.yml, and by people: 14 of its 27 commits in the window are theirs.
...
- 11 reads use paths built at run time and are not named here.
- 24 writes go to places this repository does not track, so they are not listed as generated.
- 45 writes and 170 reads go to the directory the command is run in, the home directory or a path its caller passes, not to this repository.
- 40 commands are built at run time and not followed, 35 of them in tests.
