...
12 parts, in Python (109 files), TypeScript (60 files), CSS (19 files), JavaScript (19 files), Rust (6 files), HTML (4 files), shell (4 files) and Astro (2 files). Work enters through 5 doors; the busiest is CI, which reaches 10 parts. It publishes to PyPI. It deploys a site to GitHub Pages. People run sov. People install the sov-tauri-shell desktop app.

## What changed since 2026-09-25 (41fa408)

- CI now also runs sov_daemon/__init__.py.
- Release now also runs app/src-tauri/build.rs, app/src/, app/vite.config.ts and 1 more.
...
2. **Release.** When a release is published; or by hand. Runs .github/scripts/generate-latest-json.py, .github/scripts/stage-tauri-artifacts.sh, sov_cli/main.py and 60 more; builds sov_cli/__main__.py and app/src-tauri/src/main.rs; checks sov_cli/, sov_engine/, sov_transport/ and 1 more.
...
5. **sov-tauri-shell** (the desktop app people install). Runs app/src-tauri/src/main.rs.
...
4. It scans for secrets with TruffleHog.
...
**Release** runs .github/scripts/generate-latest-json.py, .github/scripts/stage-tauri-artifacts.sh, sov_cli/main.py and 60 more, checks sov_cli/, sov_engine/, sov_transport/ and 1 more, writes to app/src-tauri/gen/schemas, which is not tracked, publishes to PyPI, and builds app/src-tauri/src/main.rs and sov_cli/__main__.py into binaries for darwin-arm64, darwin-universal, linux-x64 and win-x64 and uploads them to the release.
