...
Mapped at 2026-09-25 from commit e5fbcb9.

## What this is

Deterministic 3D RPG tick: the model proposes, a checker admits, and the host draws committed frames. (written by a person)

15 parts, mostly JavaScript (51 files), TypeScript (3) and Rust (2). Work enters through 8 doors; the busiest is CI, which reaches 8 parts. host, load, play, propose, replay and write-golden are commands of a private package (nothing ships them).
...
- 5 files added and 159 changed content, across 15 parts.

## What comes in

1. **CI.** On a pull request touching 11 paths; on a push to main touching 11 paths; or by hand. Runs harness/product.test.js, harness/solver.test.js, packages/host/host.test.js and 7 more; checks fixtures/golden-arith.txt, fixtures/golden.txt, harness/arith.mjs and 47 more.
...
**write-golden** (a command of a private package, which nothing ships) runs harness/sim.mjs and harness/write-golden.js, reaches frame and tick, and writes to fixtures/golden.txt.
...
packages/host/bin/host.js → packages/tick/admit-world.js → packages/frame/hash.js

Read those in order to follow one run of host end to end. This path follows host (a command of a private package, which nothing ships) from its entry, since CI runs only tests, scripts that import no code here and checks.

## What this map cannot see

- 5 imports could not be resolved: `harness/sim.mjs` imports `../solver/dist/solver.mjs`, which a build generates; `harness/solver.test.js` imports `../solver/dist/solver.mjs`, which a build generates; `packages/host/session.js` imports `../../solver/dist/solver.mjs`, which a build generates; and 2 more.
- 1 read uses a path built at run time and is not named here.
- 1 write goes to places this repository does not track, so it is not listed as generated.
- 3 writes and 11 reads go to a path their caller passes, not to this repository.
