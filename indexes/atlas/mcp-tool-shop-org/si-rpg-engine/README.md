...
14 parts, mostly JavaScript (44 files), Rust (2) and TypeScript (1). Work enters through 7 doors; the busiest is CI, which reaches 8 parts. People run host, load, play, propose, replay and write-golden.
...
- 1 file added and 99 changed content, across 14 parts.

## What comes in

1. **CI.** On a pull request touching 10 paths; on a push to main touching 10 paths; or by hand. Runs harness/product.test.js, harness/solver.test.js, packages/host/host.test.js and 6 more; checks fixtures/golden-arith.txt, fixtures/golden.txt, harness/arith.mjs and 41 more.
...
- 3 writes and 9 reads go to a path their caller passes, not to this repository.
- 3 writes and 4 reads go to the directory the command is run in (harness/, predicates/ and worlds/), not to this repository.
