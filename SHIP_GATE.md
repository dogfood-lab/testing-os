...
- [x] `[all]` Version in manifest matches git tag — root + 8 packages all at `1.17.0`, tag `v1.17.0` (2026-09-24)
...
- [x] `[npm]` Seven of eight `@dogfood-lab/*` packages are published on npm (six since v1.2.0: `schemas`, `verify`, `report`, `ingest`, `findings`, `dogfood-swarm`; `atlas` from v1.13.0); headline install is `npm install -g @dogfood-lab/dogfood-swarm`. The seventh (`@dogfood-lab/portfolio`) remains intentionally workspace-internal. Per-package READMEs carry the canonical logo since v1.2.1. (2026-05-14, last re-affirmed 2026-09-24 at v1.17.0)
- [x] `[npm]` `engines.node` set — root `package.json` has `"engines": {"node": ">=22"}` (tightened from `>=20` to `>=22` in v1.2.2 to match the CI Node 22 + 24 matrix; last re-affirmed 2026-09-24 at v1.17.0)
