...
Sourced primarily from [`SHIP_GATE.md`](SHIP_GATE.md): at v1.13.0 (re-affirmed 2026-09-23), every applicable hard-gate A–D row carries either an `[x]` evidence stamp or a `SKIP:` with explicit justification — 22 checked / 15 SKIP-with-justification / 0 unchecked against the current SHIP_GATE.md (the earlier "21 checked / 16 SKIP" figure tracked a different revision of the gate file; `shipcheck audit` exited 0 at the v1.9.0 release tree). Soft gate E is fully met. The "100% pass on hard gates A–D" headline phrasing reflects the audit-tool verdict, not a hand-curated estimate; the per-row evidence dates below are the auditable substrate.
...
Honest deltas: every SKIP that remains in SHIP_GATE.md (the MCP / desktop / VSCode items that don't apply, plus the real follow-ups above) is the reason individual category scores stay below 10. The repo is shippable at v1.13.0 by every applicable contract — six of seven `@dogfood-lab/*` packages are live on npm since v1.2.0; the gap is between "shippable" and "perfect."
