...
describe('what a Rust program writes and reads', () => {
  // save() is handed PathBuf::from("snapshots") by main's one call, a path
  // relative to where the command runs; the nightly workflow runs the
  // binary but commits only reports/, so snapshots/ is the person's, as
  // cache/ is, and reports/summary.txt the workflow's.
  ...
  it('credits the workflow that runs the binary with the report, and says where the rest goes', () => {
    ...
    assert.ok(data.limits.includes('2 writes go to the directory the command is run in (cache/ and snapshots/), not to this repository.'), data.limits.join('\n'));
