...
Paths built at run time are counted, not named. Imports that do not resolve are counted with their reason, and so are imports of paths outside the repository, writes to untracked files, and writes to the directory the command is run in; a claim that nothing writes or imports is qualified by the files the parser could not read. The page ends with the list of what the map could not see for that repository, so a reader knows the edges of the picture.
