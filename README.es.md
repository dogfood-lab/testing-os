...
<!-- version:start -->
**v1.18.0** — versión actual. Consulte [CHANGELOG.md](CHANGELOG.md) para ver las novedades.
<!-- version:end -->
...
`testing-os` registra, verifica y aprende de las pruebas reales de su repositorio en un flujo de trabajo nativo de IA. Apúntelo a un repositorio y cada ejecución de prueba se convierte en un registro con confirmación de procedencia en el que puede confiar, y no en un resultado autoinformado.

Lo que obtendrá:

- **Registros con confirmación de procedencia.** Cada envío está vinculado a una ejecución de CI real, sin necesidad de claves, a través de la identidad del propio proveedor, antes de que se acepte. El resultado es un almacén de pruebas inmutable y de solo anexión, no una simple marca verde basada en la confianza.
...
**Las transiciones de estado de descubrimiento son evidencia y solo se pueden agregar.** Los verbos de cierre del plano de control del enjambre (`swarm reopen`, `swarm close`) requieren una razón explícita, evidencia y, para los cierres de operador, un modo de verificación declarado; cada transición escribe una fila `finding_events` inmutable que registra la autoridad que actúa. Ninguna ruta automatizada puede cerrar un descubrimiento por inactividad o volver a abrirlo por predicción, y ningún verbo puede reescribir el historial de eventos; una credencial utilizada incorrectamente puede agregar transiciones, pero cada adición se registra.

**Superficie de red.** De forma predeterminada, la única salida es `api.github.com` (solo lectura: confirmación del origen + la recuperación de la definición del escenario mencionada anteriormente). Las dos excepciones son ambas opcionales y se describen anteriormente: un envío de proveedor de GitLab (`gitlab.com/api`) y una ejecución de ancla XRPL habilitada por el operador. **Sin telemetría, sin análisis: esta base de código nunca se comunica con el exterior; si no se utilizan esas dos rutas opcionales, no expone ninguna superficie de red más allá de GitHub.** El flujo de trabajo del receptor se ejecuta con `contents: write`, que solo tiene ámbito en este repositorio.
...
| `@dogfood-lab/ingest` | JS | Conexión de canalización: envío → verificación → persistencia → indexación. |
