...
<!-- version:start -->
**v1.22.0** — versión actual. Consulte [CHANGELOG.md](CHANGELOG.md) para ver qué se ha incluido.
<!-- version:end -->
...
- **Un contrato de políticas que usted controla.** Declare qué cuenta como "verificado" en YAML: un DSL de predicados acotado y sin evaluación (`field`/`op`/`value` + `all`/`any`/`not`/`implies`), y aplíquelo en todos sus repositorios. Analice una política antes de enviarla con `dogfood-verify lint`.
...
| `@dogfood-lab/findings` | JS | Contrato de detección + derivación/revisión/síntesis/líneas de asesoramiento. |
