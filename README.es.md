...
<!-- version:start -->
**v1.21.0** — versión actual. Consulte [CHANGELOG.md](CHANGELOG.md) para ver las novedades.
<!-- version:end -->
...
`testing-os` registra, verifica y aprende de la evidencia de pruebas real de su repositorio en un flujo de trabajo nativo de IA. Apúntelo a un repositorio y cada ejecución de prueba se convierte en un registro con confirmación de procedencia en el que puede confiar, y no en un resultado autoinformado.
...
Es el monorepositorio insignia de la organización [Dogfood Lab](https://github.com/dogfood-lab): ocho `@dogfood-lab/*` paquetes detrás de una `swarm` CLI y una `atlas` CLI.
...
testing-os procesa los envíos de dogfood enviados a través de `repository_dispatch` desde repositorios de GitHub de confianza bajo `mcp-tool-shop-org/*` y `dogfood-lab/*`. El verificador requiere la procedencia de la CI: los ID de ejecución declarados se confirman a través de la API del proveedor, y los envíos con formas incorrectas, referencias faltantes o afirmaciones de políticas no válidas se rechazan.
