...
| What comes in | Every workflow: what triggers it, what its steps run and what they only check (linters, type-checkers), read by the conventions of the tools they name (`npm run`, `pnpm` and `yarn` scripts with their workspace filters, `pytest`, `uv run`, `python -m`, `tsc --build`, `vitest`, `eslint`, shell scripts, Makefile targets, spawned commands, including commands a helper hands to a shell with literal text, and a run of a build output such as `node dist/cli.js`, traced to its source through the tsconfig the build names). Then every command a manifest installs (a `bin` in `package.json`, a `[project.scripts]` entry in `pyproject.toml`) and the entry of the package, said as published only when a door publishes it, each a door of its own. A container, wheel or binary build runs the file it starts (a Dockerfile's `ENTRYPOINT` or `CMD`, traced through stages, `WORKDIR` and `COPY`) and checks the files it packs. A child process that starts a module of this repository (`python -m <package>`, `node <file>`) connects the parts; `git` and `gh` are said as "runs git". A tracked file handed to a tool the map has no rule for is a checked run, a `cd` moves the rest of the step, and a job or step gated by an `if` on the event, the ref, its type or an input, or held off one (`!=`), keeps what it runs, deploys or commits to the events that remain ("deploys the site on a push to main"; "when run by hand with dry_run false, it also publishes"). A publish names what it sends: a package chosen by its directory or by the tag, the VS Code Marketplace or Open VSX for an extension, nothing for a dry run. |
| What happens through the busiest door | The parts its executed files are in, the parts reached through imports in order, the order of work inside the entry functions, the places it writes (a written-out file name lands on that file, not its directory; a path built from the directory the command is run in, from the home directory, from a helper that returns one, from an environment variable or from an argument the caller passes is not a place here; a write into a file the repository does not track is counted, never placed; a write skipped in CI, or behind a flag every run passes, is not credited to the door), what it commits and pushes, here, to a branch for review, or into a clone of another repository, what it changes in other repositories through the GitHub API, and what it dispatches, publishes or deploys. What a door only checks reaches parts but writes nothing. |
| Who reads the results | The files and workflows that read those places: code that reads or imports them, the site build that renders them, and tests, counted after the code; readers found by scanning text are marked and never counted as structure; a Markdown link, a path mentioned in prose, and a packaging list are not readers. |
...
| What no test touches | Code parts no test file imports, directly or through one hop, including tests that spawn scripts; data and site parts are never asked. A test that rewrites tracked files is a writer of them, listed under Generated as "(a test)". |
...
| Generated, never hand-edited | Places with a writer in code or in a workflow whose commits are not mostly people's; a tracked file its writer reads first is said to have a block written by it and stays mixed; an untracked output is not a place. |
| Hand-authored | Parts nothing in the repository writes to, with the count of run-time-path writes that may land there; and places a workflow writes that people commit to more ("written by X, and by people: 67 of its 68 commits in the window are theirs"). |
| Where to start | A chain of files that run, from the door a pull request goes through whenever the busiest door is not it (release, deploy and schedule triggers included), beginning at that door's entry point rather than the first helper it names, to the first reader of its output; every arrow is a recorded edge (a run, an import, a write, a read), every step names a file, files with no statements, manifests and data files are skipped, a test is passed over for the production file it imports, and the chain ends in code. When nothing can be followed it says so. |
| What this map cannot see | Unresolved imports, imports of paths outside the repository, declared dependencies that share a name with a local module, files the parser cannot read (named, up to three, then counted by part with the construct that stopped it), paths built at run time, writes to the directory the command is run in, the home directory or a path the caller passes, writes to files the repository does not track, commands built at run time (with how many of them are in tests), text-scanned readers, and the confidence of the history statistics. |
