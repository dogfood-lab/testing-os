...
| What comes in | Every workflow: what triggers it, what its steps run and what they only check (linters, type-checkers), read by the conventions of the tools they name (`npm run`, `pnpm` and `yarn` scripts with their workspace filters, `pytest`, `uv run`, `python -m`, `tsc --build`, `vitest`, `eslint`, shell scripts, Makefile targets, spawned commands, including commands a helper hands to a shell with literal text, and a run of a build output such as `node dist/cli.js`, traced to its source through the tsconfig the build names). Then every command a manifest installs (a `bin` in `package.json`, a `[project.scripts]` entry in `pyproject.toml`) and the entry of the package, said as published only when a door publishes it, each a door of its own. A container, wheel or binary build runs the file it starts and checks the files it packs. A tracked file handed to a tool the map has no rule for is a checked run, a `cd` moves the rest of the step, and a job gated to one event or branch, or held off one (`!=`), keeps what it deploys or commits to the events that remain ("deploys the site on a push to main"). |
...
| What no test touches | Code parts no test file imports, directly or through one hop, including tests that spawn scripts; data and site parts are never asked. |
...
| Where to start | A chain of files that run, from the door a pull request goes through when the busiest door fires only on a push or a schedule, to the first reader of its output; every arrow is a recorded edge (a run, an import, a write, a read), every step names a file, files with no statements, manifests and data files are skipped, a test is passed over for the production file it imports, and the chain ends in code. When nothing can be followed it says so. |
