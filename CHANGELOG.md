...
- **Checked is not executed.** Each run a door makes records `runKind`: `executes` for program runners and test runners, `checks` for eslint, ruff, mypy, black, flake8, pylint, bandit and tsc (always, since a compiler runs none of the code it builds). Reach is walked from both, but a door's landings, readers, "It writes to …" and the order of work come only from what it executes; the page says "Runs X; checks Y", `explain` says "Checked by", and the delta says "now also checks". ai-rpg-engine's CI had been said to write to a scripts directory because `eslint .` matched every file in the repository.

### Fixed
- **A file write lands on the file, not its directory.** A written-out path that names a file, tracked or not, lands on that file when a tracked directory holds it; only a call that makes or lists a directory, a trailing `/`, a tail built at run time, or a path through `dist/` or `node_modules/` still lands on a directory. `new URL(…, import.meta.url)` with `.pathname` or `.href` resolves relative to the file. backpropagate's mutmut door had been said to write `.github/` for one baseline file, and ai-rpg-engine's replay script to write its whole `scripts/` directory for one receipt.
...
- **The limits tell tests and fixtures apart.** "272 commands are built at run time and not followed, 251 of them in tests" and "9 files use syntax the parser cannot read …: 8 in fixtures (…), 1 in schemas (…)" replace two true lines that misled at a glance.
