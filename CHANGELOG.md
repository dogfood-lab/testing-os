...
### Changed
- **Doors on the page say what they do.** "publishes to PyPI and a container image", "opens an issue when it fails", "opens a pull request", and "a path set at run time" in place of a variable. A pull request trigger counts its paths, a release trigger reads by its types, and a push filtered by branches and tags names both. A run under a directory the door also runs is not named again, what the commands name comes first, a directory spanning parts says how many, and "and N more" counts what the artifact capped. The busiest door reaches the most parts, never none, and a schedule-only door loses a tie: on backpropagate that is CI, not the weekly smoke, and on this repository it is CI, which reaches eleven parts, where it was the ingest door. The site reads `runsCount` from `page.json` and names three runs of the main door's first step and counts the rest.
