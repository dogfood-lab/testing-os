...
- **एक पृष्ठ जो बताता है कि एक रिपॉजिटरी कैसे काम करता है।** एटलस एक रिपॉजिटरी के वर्कफ़्लो और मेनिफेस्ट को पढ़ता है, वे उपकरण जो यह चलाता है, इसकी आयात, लेखन और इतिहास, और `atlas/README.md` लिखता है: क्या आता है, क्या चलता है, यह कहां उतरता है, कौन इसे पढ़ता है, क्या क्या तोड़ता है, पिछली मानचित्र से क्या बदला है, कहां से शुरू करें। इसमें कोई भी वाक्य किसी व्यक्ति द्वारा नहीं लिखा गया है; `atlas check` तब सीआई विफल हो जाता है जब मानचित्र कोड से मेल खाना बंद कर देता है, `atlas explain` उत्तर देता है कि एक फ़ाइल, निर्देशिका या भाग सिस्टम में क्या है, `atlas mcp` एक एजेंट को सत्र के दौरान मानचित्र से समान प्रश्न पूछने देता है, और प्रत्येक पुल अनुरोध को एक टिप्पणी के रूप में संरचनात्मक डेल्टा मिलता है।

यह [डॉगफूड लैब](https://github.com/dogfood-lab) संगठन का प्रमुख मोनोरिपॉजिटरी है - आठ `@dogfood-lab/*` पैकेज एक `swarm` सीएलआई और एक `atlas` सीएलआई के पीछे।
...
`./atlas-data` मेमोरी है: `fleet.yml`, प्रत्येक रेंडर, प्रत्येक रिपॉजिटरी का इतिहास और स्थिति। सेवा एक बार शुरू में मेमोरी खाली होने पर मैप करती है, फिर `fleet.yml` में शेड्यूल पर, और `http://127.0.0.1:8080/` पर बेड़े की सूची, प्रत्येक पृष्ठ `/?repo=owner/name` पर और एजेंटों के लिए एक इंडेक्स `/llms.txt` पर प्रस्तुत करती है। आपके द्वारा सूचीबद्ध रिपॉजिटरी के गिट फ़ेच को छोड़कर कुछ भी कंटेनर को नहीं छोड़ता है; `./atlas-data` को हटाना ही भूलने का एकमात्र तरीका है। समान छवि एक रिपॉजिटरी पर सीएलआई चलाती है: `docker run --rm -v "$PWD:/repo" ghcr.io/dogfood-lab/atlas map`। कमांड चलाएं और फ़ाइल आकार [`docker/README.md`](docker/README.md) में हैं।
...
**साक्ष्य ही प्रमाण है।** `github` सबमिशन के लिए, सत्यापनकर्ता पुष्टि करता है कि दावा किया गया GitHub क्रियाएं रन वास्तव में मौजूद है (GitHub API) और सबमिशन के `repo` और `commit_sha` को उस पुष्टि किए गए रन से जोड़ता है - एक लाइव, बिना कुंजी वाला जांच जो GitHub की अपनी ओआईडीसी पहचान में निहित है, इसलिए एक रिकॉर्ड किसी ऐसे रन या कमिट का प्रमाण नहीं दे सकता है जो नहीं हुआ। **गिटलैब सीआई** वैकल्पिक रूप से समर्थित है (`source.provider: gitlab`); एक गिटलैब सबमिशन एकमात्र मामला है जहां सत्यापनकर्ता एक गैर-GitHub होस्ट को कॉल करता है (`gitlab.com/api`), और केवल `gitlab` सबमिशन के लिए।

**रिकॉर्ड अखंडता छेड़छाड़-रोधी है, छेड़छाड़-प्रूफ नहीं।** प्रत्येक बने रहने वाले रिकॉर्ड में एक `integrity` ब्लॉक होता है (`submission_digest` + `prev_digest`) जो एक केवल-जोड़ने वाली हैश श्रृंखला बनाता है जिसे `node packages/ingest/run.js --verify-chain` पूरी तरह से ऑफ़लाइन में मान्य करता है - बैंड से बाहर छेड़छाड़, डिस्क भ्रष्टाचार और आंशिक पुनर्स्थापना का पता लगाता है। यह स्वयं इनजेस्ट क्रेडेंशियल के खिलाफ बचाव नहीं करता है, जो एक रिकॉर्ड और श्रृंखला दोनों को फिर से लिख सकता है; इसे बंद करने के लिए लेखक के नियंत्रण के बाहर एक एंकर की आवश्यकता होती है। एक **वैकल्पिक, डिफ़ॉल्ट रूप से बंद एक्सआरपीएल एंकर** (`node packages/ingest/run.js --anchor-*`) श्रृंखला के शीर्ष को सार्वजनिक एक्सआरपी लेजर में प्रस्तुत करता है, जिससे एंकर किए गए बिंदु के नीचे किसी भी संक्षिप्तीकरण या पुनर्लेखन का पता लगाया जा सकता है - गैर-GitHub कॉल, और केवल तभी जब एक ऑपरेटर इसे सक्षम करता है।

**टेस्टिंग-ओएस किन चीज़ों को प्रभावित करता है:** प्रत्येक `repository_dispatch` पेलोड में सबमिशन JSON; इस रिपॉजिटरी में `policies/`, `fixtures/`, `records/`, `indexes/` और `dogfood/roadmap/` (अंतिम केवल ऑपरेटर-द्वारा सक्रिय `swarm roadmap compile` द्वारा लिखा जाता है — कभी भी स्वचालित इनजेस्ट पथ द्वारा नहीं); `api.github.com` के लिए आउटबाउंड कॉल, ताकि इसकी प्रामाणिकता की पुष्टि की जा सके; और — केवल `github` सबमिशन के लिए — सबमिट किए गए रिपॉजिटरी के `dogfood/scenarios/<scenario_id>.yaml` की रीड-ओनली फ़ेच, जो प्रमाणित कमिट पर होती है (परिदृश्य परिभाषा जो आवश्यक चरणों को लागू करती है; उपयोग से पहले आकार-सीमित और स्कीमा-मान्य, अनुपस्थित फ़ाइलें केवल उस जाँच को अनदेखा कर देती हैं और एक दृश्यमान चेतावनी प्रदर्शित करती हैं)।

**टेस्टिंग-ओएस किन चीज़ों को प्रभावित नहीं करता है:** घोषित `dogfood/scenarios/` परिभाषा फ़ाइलों से परे उपभोक्ता स्रोत कोड, उपभोक्ता रिपॉजिटरी में गुप्त जानकारी, या इस रिपॉजिटरी के वर्किंग ट्री के बाहर की कोई भी चीज़।

**खोज-अवस्था परिवर्तन प्रमाण-आधारित होते हैं और केवल जोड़ने की अनुमति देते हैं।** स्वार्म नियंत्रण तल के समापन क्रियाएँ (`swarm reopen`, `swarm close`) को एक स्पष्ट कारण, प्रमाण और — ऑपरेटर समापन के लिए — एक घोषित सत्यापन मोड की आवश्यकता होती है; प्रत्येक परिवर्तन एक अपरिवर्तनीय `finding_events` पंक्ति लिखता है जो कार्य करने वाले प्राधिकरण को रिकॉर्ड करता है। कोई भी स्वचालित पथ पुरानी होने पर किसी खोज को बंद नहीं कर सकता है या भविष्यवाणी के आधार पर उसे फिर से नहीं खोल सकता है, और कोई भी क्रिया घटना इतिहास को फिर से नहीं लिख सकती है — गलत तरीके से उपयोग किए गए क्रेडेंशियल परिवर्तन जोड़ सकते हैं, लेकिन प्रत्येक जोड़ स्वयं रिकॉर्ड में होता है।

**नेटवर्क सतह।** डिफ़ॉल्ट रूप से, एकमात्र आउटगोइंग कनेक्शन `api.github.com` है (केवल-पढ़ने के लिए: प्रामाणिकता की पुष्टि + ऊपर दी गई परिदृश्य-परिभाषा फ़ेच)। दो अपवाद दोनों वैकल्पिक हैं और ऊपर बताए गए हैं: एक GitLab-प्रदाता सबमिशन (`gitlab.com/api`), और एक ऑपरेटर-सक्षम XRPL एंकर रन। **कोई टेलीमेट्री नहीं, कोई एनालिटिक्स नहीं — यह कोडबेस कभी भी घर नहीं भेजता है; उन दो वैकल्पिक पथों के अभाव में, यह GitHub से परे कोई नेटवर्क सतह उजागर नहीं करता है।** रिसीवर वर्कफ़्लो `contents: write` के साथ चलता है, जो केवल इस रिपॉजिटरी तक सीमित है।
...
| `@dogfood-lab/atlas` | JS | एक रिपॉजिटरी पढ़ता है और उस पृष्ठ को लिखता है जिसमें बताया गया है कि यह कैसे काम करता है (`atlas/README.md`); `atlas check` CI में मानचित्र को नियंत्रित करता है, और `atlas mcp` इससे एजेंटों को उत्तर देता है। कोई भी संबंधित निर्भरताएँ नहीं; किसी भी रिपॉजिटरी में चलता है। |
...
**समर्थित फ़ाइल सिस्टम:** APFS, HFS+, ext4 (CI आधारभूत), NTFS — कोई भी जो POSIX `link(2)` को लागू करता है। **समर्थित नहीं:** exFAT, FAT32। [`packages/findings/lib/file-lock.js`](packages/findings/lib/file-lock.js) में फ़ाइल-लॉक CAS को परमाणु प्रकाशन के लिए हार्डलिंक सिमेंटिक्स की आवश्यकता होती है; exFAT पर, `linkSync` `ENOTSUP` उत्पन्न करता है (जोरदार, मौन नहीं)। सामान्य समस्या: क्रॉस-प्लेटफ़ॉर्म बाहरी SSD अक्सर exFAT में स्वरूपित होते हैं — रिपॉजिटरी को स्थानीय APFS/HFS+ में क्लोन करें। पूर्ण सत्र G सत्यापन मैट्रिक्स के लिए [`docs/m5-validation-2026-04-29.md`](docs/m5-validation-2026-04-29.md) देखें।
