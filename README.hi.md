...
<!-- version:start -->
**v1.16.0** — वर्तमान रिलीज़। इसमें क्या शामिल किया गया है, यह देखने के लिए [CHANGELOG.md](CHANGELOG.md) देखें।
<!-- version:end -->
...
- **एक नीति अनुबंध जिसे आप नियंत्रित करते हैं।** YAML में घोषित करें कि "सत्यापित" के रूप में क्या गिना जाता है - एक सीमित, गैर-मूल्यांकन पूर्वानुमान डीएसएल (`field`/`op`/`value` + `all`/`any`/`not`/`implies`) - और इसे अपने रिपॉजिटरी में लागू करें। `dogfood-verify lint` के साथ शिप करने से पहले एक नीति को जांचें।
...
सार्वजनिक साइट हर सार्वजनिक रिपॉजिटरी को प्रस्तुत करती है जिसने एटलस को अपनाया है। उन रिपॉजिटरी के लिए जिन्हें आपके मशीन को नहीं छोड़ना चाहिए, समान इंजन एक कंटेनर के रूप में स्थायी मेमोरी के साथ भेजा जाता है:
...
**टेस्टिंग-ओएस किन चीज़ों को प्रभावित करता है:** प्रत्येक `repository_dispatch` पेलोड में सबमिशन JSON; इस रिपॉजिटरी में `policies/`, `fixtures/`, `records/`, `indexes/` और `dogfood/roadmap/` (अंतिम केवल ऑपरेटर-द्वारा सक्रिय `swarm roadmap compile` द्वारा लिखा जाता है - स्वचालित रूप से सबमिट करने वाले रास्ते से नहीं); `api.github.com` पर आउटबाउंड कॉल, ताकि इसकी प्रामाणिकता की पुष्टि की जा सके; और - केवल `github` सबमिशन के लिए - सबमिट करने वाली रिपॉजिटरी के `dogfood/scenarios/<scenario_id>.yaml` की रीड-ओनली फ़ेच, जो प्रमाणित कमिट पर होती है (परिदृश्य परिभाषा जो आवश्यक चरणों को लागू करती है; उपयोग से पहले आकार-सीमित और स्कीमा-मान्य, अनुपस्थित फ़ाइलें केवल उस जाँच को लागू नहीं होने देती हैं और एक दृश्यमान चेतावनी प्रदर्शित करती हैं)।

**टेस्टिंग-ओएस किन चीज़ों को प्रभावित नहीं करता है:** घोषित `dogfood/scenarios/` परिभाषा फ़ाइलों से परे उपभोक्ता स्रोत कोड, उपभोक्ता रिपॉजिटरी में गुप्त जानकारी, या इस रिपॉजिटरी के कार्यक्षेत्र के बाहर की कोई भी चीज़।

**खोज-अवस्था परिवर्तन प्रमाण-आधारित होते हैं और केवल जोड़े जा सकते हैं।** स्वार्म नियंत्रण विमान के समापन क्रियाएँ (`swarm reopen`, `swarm close`) को एक स्पष्ट कारण, प्रमाण और - ऑपरेटर समापन के लिए - एक घोषित सत्यापन मोड की आवश्यकता होती है; प्रत्येक परिवर्तन एक अपरिवर्तनीय `finding_events` पंक्ति लिखता है जो कार्य करने वाले प्राधिकरण को रिकॉर्ड करता है। कोई भी स्वचालित रास्ता पुरानी होने पर किसी खोज को बंद नहीं कर सकता है या भविष्यवाणी के आधार पर उसे फिर से नहीं खोल सकता है, और कोई भी क्रिया घटना इतिहास को फिर से नहीं लिख सकती है - गलत तरीके से उपयोग किए गए क्रेडेंशियल परिवर्तन जोड़ सकते हैं, लेकिन प्रत्येक जोड़ स्वयं रिकॉर्ड पर होता है।

**नेटवर्क सतह।** डिफ़ॉल्ट रूप से, एकमात्र आउटगोइंग कनेक्शन `api.github.com` है (केवल-पढ़ने के लिए: प्रामाणिकता की पुष्टि + ऊपर दी गई परिदृश्य-परिभाषा फ़ेच)। दो अपवाद दोनों वैकल्पिक हैं और ऊपर बताए गए हैं: एक GitLab-प्रदाता सबमिशन (`gitlab.com/api`), और एक ऑपरेटर-सक्षम XRPL एंकर रन। **कोई टेलीमेट्री नहीं, कोई विश्लेषण नहीं - यह कोडबेस कभी भी घर नहीं भेजता है; उन दो वैकल्पिक रास्तों के अभाव में, यह GitHub से परे कोई नेटवर्क सतह उजागर नहीं करता है।** रिसीवर वर्कफ़्लो `contents: write` के साथ चलता है, जो केवल इस रिपॉजिटरी तक सीमित है।
...
**समर्थित फ़ाइल सिस्टम:** APFS, HFS+, ext4 (CI आधार रेखा), NTFS - कोई भी जो POSIX `link(2)` को लागू करता है। **समर्थित नहीं:** exFAT, FAT32। [`packages/findings/lib/file-lock.js`](packages/findings/lib/file-lock.js) में फ़ाइल-लॉक CAS को परमाणु प्रकाशन के लिए हार्डलिंक सिमेंटिक्स की आवश्यकता होती है; exFAT पर, `linkSync` `ENOTSUP` उत्पन्न करता है (जोरदार, मौन नहीं)। एक सामान्य समस्या: क्रॉस-प्लेटफ़ॉर्म बाहरी SSD अक्सर exFAT में स्वरूपित होते हैं - इसके बजाय रिपॉजिटरी को स्थानीय APFS/HFS+ पर क्लोन करें। पूर्ण सत्र G सत्यापन मैट्रिक्स के लिए [`docs/m5-validation-2026-04-29.md`](docs/m5-validation-2026-04-29.md) देखें।

## संस्करण

सभी `@dogfood-lab/*` पैकेज एक साथ अपडेट होते हैं - पूरे मोनोरेपो में एक संख्या। सात पैकेज लॉकस्टेप में `@dogfood-lab` के तहत npm पर v1.16.0 पर प्रकाशित होते हैं (`schemas`, `verify`, `report`, `ingest`, `findings`, `dogfood-swarm`, `atlas`); आठवां, `@dogfood-lab/portfolio`, आंतरिक रहता है। इस README के शीर्ष के पास संस्करण पंक्ति हर `npm run build` पर [`scripts/sync-version.mjs`](scripts/sync-version.mjs) के माध्यम से `package.json` से स्वचालित रूप से स्टैम्प की जाती है।
