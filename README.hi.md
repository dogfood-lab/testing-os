...
<!-- version:start -->
**v1.13.0** — वर्तमान रिलीज़। इसमें क्या शामिल किया गया है, यह देखने के लिए [CHANGELOG.md](CHANGELOG.md) देखें।
<!-- version:end -->
...
`testing-os` आपके रिपॉजिटरी के वास्तविक परीक्षण प्रमाणों को एआई-आधारित वर्कफ़्लो में रिकॉर्ड, सत्यापित और सीखता है। इसे किसी रिपॉजिटरी पर इंगित करें, और प्रत्येक परीक्षण रन एक प्रमाण-पुष्टि रिकॉर्ड बन जाता है जिस पर आप भरोसा कर सकते हैं - यह स्व-रिपोर्टेड पास नहीं है।
...
- **एक पृष्ठ जो बताता है कि एक रिपॉजिटरी कैसे काम करता है।** एटलस एक रिपॉजिटरी के वर्कफ़्लो, आयात, लेखन और इतिहास को पढ़ता है और `atlas/README.md` लिखता है: क्या आता है, क्या चलता है, यह कहां उतरता है, कौन इसे पढ़ता है, क्या क्या तोड़ता है, कहां से शुरू करें। इसमें कोई भी वाक्य किसी व्यक्ति द्वारा नहीं लिखा गया है, और `atlas check` तब सीआई विफल हो जाता है जब मानचित्र कोड से मेल खाना बंद कर देता है।
...
**रिकॉर्ड अखंडता छेड़छाड़-रोधी है, छेड़छाड़-प्रूफ नहीं।** प्रत्येक बने रहने वाले रिकॉर्ड में एक `integrity` ब्लॉक होता है (`submission_digest` + `prev_digest`) जो एक केवल-जोड़ने वाली हैश श्रृंखला बनाता है जिसे `node packages/ingest/run.js --verify-chain` पूरी तरह से ऑफ़लाइन में मान्य करता है - बैंड से बाहर छेड़छाड़, डिस्क भ्रष्टाचार और आंशिक पुनर्स्थापना का पता लगाता है। यह स्वयं अंतर्ग्रहण क्रेडेंशियल के खिलाफ बचाव नहीं करता है, जो एक रिकॉर्ड और श्रृंखला दोनों को फिर से लिख सकता है; इसे बंद करने के लिए लेखक के नियंत्रण के बाहर एक एंकर की आवश्यकता होती है। एक **वैकल्पिक, डिफ़ॉल्ट रूप से बंद XRPL एंकर** (`node packages/ingest/run.js --anchor-*`) श्रृंखला के शीर्ष को सार्वजनिक एक्सआरपी लेज़र में गवाह बनाता है, जिससे एंकर किए गए बिंदु के नीचे किसी भी संक्षिप्तीकरण या पुनर्लेखन का पता लगाया जा सकता है - गैर-GitHub कॉल, और केवल तभी जब कोई ऑपरेटर इसे सक्षम करता है।

**टेस्टिंग-ओएस क्या छूता है:** प्रत्येक `repository_dispatch` पेलोड में सबमिशन JSON; इस रिपॉजिटरी में `policies/`, `fixtures/`, `records/`, `indexes/` और `dogfood/roadmap/` (अंतिम केवल ऑपरेटर-ट्रिगर `swarm roadmap compile` द्वारा लिखा जाता है - कभी भी स्वचालित अंतर्ग्रहण पथ द्वारा नहीं); प्रमाण सत्यापन के लिए `api.github.com` पर आउटबाउंड कॉल; और - केवल `github` सबमिशन के लिए - सबमिट करने वाले रिपॉजिटरी के `dogfood/scenarios/<scenario_id>.yaml` की केवल-पढ़ने वाली फ़ेच, जिस पर प्रमाणित कमिट है (परिदृश्य परिभाषा जो आवश्यक-चरण प्रवर्तन को शक्ति प्रदान करती है; उपयोग करने से पहले आकार-सीमित और स्कीमा-मान्य, अनुपस्थित फ़ाइलें केवल उस जांच को दृश्यमान चेतावनी के साथ लागू नहीं करती हैं)।

**टेस्टिंग-ओएस क्या नहीं छूता है:** घोषित `dogfood/scenarios/` परिभाषा फ़ाइलों से परे उपभोक्ता स्रोत कोड, उपभोक्ता रिपॉजिटरी में गुप्त जानकारी, या इस रिपॉजिटरी के कार्यशील ट्री के बाहर कुछ भी।

**खोज-अवस्था संक्रमण प्रमाण-आधारित और केवल-जोड़ने वाले होते हैं।** स्वार्म नियंत्रण विमान के समापन क्रियाओं (`swarm reopen`, `swarm close`) के लिए एक स्पष्ट कारण, प्रमाण और - ऑपरेटर समापन के लिए - एक घोषित सत्यापन मोड की आवश्यकता होती है; प्रत्येक संक्रमण एक अपरिवर्तनीय `finding_events` पंक्ति लिखता है जो अभिनय प्राधिकरण को रिकॉर्ड करता है। कोई भी स्वचालित पथ पुरानी होने पर खोज को बंद नहीं कर सकता है या भविष्यवाणी द्वारा इसे फिर से नहीं खोल सकता है, और कोई भी क्रिया इतिहास को फिर से नहीं लिख सकता है - गलत तरीके से उपयोग किया गया क्रेडेंशियल संक्रमण जोड़ सकता है, लेकिन प्रत्येक जोड़ स्वयं रिकॉर्ड पर है।

**नेटवर्क सतह।** डिफ़ॉल्ट रूप से, एकमात्र आउटगोइंग कनेक्शन `api.github.com` है (केवल-पढ़ने के लिए: उत्पत्ति की पुष्टि + ऊपर वर्णित परिदृश्य-परिभाषा प्राप्त करना)। दो अपवाद दोनों वैकल्पिक हैं और ऊपर बताए गए हैं: एक GitLab-प्रदाता सबमिशन (`gitlab.com/api`), और एक ऑपरेटर-सक्षम XRPL एंकर रन। **कोई टेलीमेट्री नहीं, कोई विश्लेषण नहीं - यह कोडबेस कभी भी स्वचालित रूप से डेटा नहीं भेजता; उन दो वैकल्पिक रास्तों के अभाव में, यह GitHub से परे कोई नेटवर्क सतह उजागर नहीं करता है।** रिसीवर वर्कफ़्लो `contents: write` के साथ चलता है, जो केवल इस रिपॉजिटरी तक सीमित है।
...
सभी `@dogfood-lab/*` पैकेज एक साथ अपडेट होते हैं - पूरे मोनोरेपो में एक संख्या। सात पैकेज v1.13.0 पर `@dogfood-lab` के तहत एक साथ npm पर प्रकाशित होते हैं (`schemas`, `verify`, `report`, `ingest`, `findings`, `dogfood-swarm`, `atlas`); आठवां, `@dogfood-lab/portfolio`, आंतरिक रहता है। इस README के शीर्ष के पास संस्करण पंक्ति हर `npm run build` पर [`scripts/sync-version.mjs`](scripts/sync-version.mjs) के माध्यम से `package.json` से स्वचालित रूप से मुद्रित होती है।
