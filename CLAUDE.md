...
### Versioning
**Lockstep.** All packages bump together. Currently **`1.23.0`** ([release v1.23.0 cut 2026-09-26 — Atlas answers agents: `atlas mcp`, a stdio MCP sidecar with six read-only tools over the committed map (provenance, a basis on every fact, 8 KB caps, no network, no model); the `llms.txt` agent index beside fleet.json and the no-JavaScript site pointer; the engine stamped on every map; explain for parts and places; coverage and test results to Codecov from CI]; [release v1.22.0 cut 2026-09-25 — the map holds on the last two waves: seventy-eight repositories on the site; workflow script text no longer republished in the map, unittest, command loops, matrix working directories, dotnet and npm run arguments read, Astro frontmatter, file: self-dependencies, sys.path and namespace packages resolved, shipped actions and published bins as doors, unparsed languages counted, every start-chain arrow an import or a call]; [release v1.21.0 cut 2026-09-25 — the map holds on the seventh and eighth waves: sixty-six repositories on the site; the start chain inside a part, turbo, Next, pnpm tool binaries, vitest projects, const import tables, local reusable workflows, Python spawns, tsup and emitting tsc as builds, positive gates, tracked dist/, publishes by member, named unresolved imports, setuptools roots, sites named by part]; [release v1.20.0 cut 2026-09-25 — the map holds on the sixth wave: forty-six repositories on the site; release builds with their assets named, Cargo examples as doors, build scripts and `gen/schemas/`, bins nothing ships, a Rust binary into its own library, untracked directories named, caller places by kind through Rust fields and parameters, paths through a job's own checkout, fork gates, Godot run-time test discovery, every language on the first line, manifests as reads]; [release v1.19.0 cut 2026-09-24 — six languages: Rust and GDScript read at the standard of the other four (crates as parts with their bins and Tauri desktop apps as doors, the module tree, cargo runs and checks, `fs` landings, unit-test files; `project.godot` with the main scene as the game door, `res://` resolution, Godot/GUT/gdUnit4/gdtoolkit runs, exports as sends, `user://` counted apart), a job's own checkout path read from inside it, and "Where to start" passing over what a door only checks]; [release v1.18.0 cut 2026-09-24 — the map holds on the fourth and fifth waves: forty-one repositories on the site, the wave-4 and wave-5 regressions fixed at the class (chains that never follow a check, ties stated, caller paths, test spawns, bootstrap writes, main-guarded writes, temporary roots, files read as git stores them) and the new classes (shell-expanded globs with the tests they leave out, built-output globs, configuration readers, HTTP edges, unseen files, bundler outputs traced to their entry, `git add` as no write, Hugging Face and Zenodo publishes, entries that are no library, bundled bins, more parser shapes, vite runs and shipped-by-hand kinds, literal loops, `init` ignore lines)]; [release v1.17.0 cut 2026-09-24 — the map holds on the third wave: twenty-three repositories on the site, the wave-3 regressions fixed (workspace members resolve with no install, Dockerfile entrypoints, publishers named, job and step gates, parameter-rooted writes, chains from the pull-request door's entry, constructs named and absence claims qualified) and five new classes (callbacks, spawns as edges, tests as writers, every language counted with image packs as data, prose)]; [release v1.16.0 cut 2026-09-24 — the map holds on the second wave: sixteen repositories on the site, the wave-2 regressions fixed (a map independent of what is built, build outputs as their source, pnpm and yarn scripts, home and caller paths in every form, negated gates, syntax checks, prose not a reader) and eleven new classes (tests as readers, helper spawns, artifact builds, manifest reads, side-branch pushes, published doors, every export, who-commits, named parse failures, a `data` role, recorded-edge chains); the swarm drafts, briefs, verifies and gates from the map]; [release v1.15.0 cut 2026-09-23 — Atlas runs a fleet: the container with persistent memory and the first GHCR push, eleven repositories on the site, the fleet-pilot and wave-1 defect classes fixed (command doors, checks apart from runs, file-level writes, clone commits, caller-relative writes, host-independent resolution, readers that are readers), the weekly job re-rendering on engine change, and the open items closed (part labels, default-branch edit link, floor hysteresis, capped-door deltas, parallel-safe suite)]; [release v1.14.0 cut 2026-09-23 — Atlas as an instrument: what changed since the last map, the pull-request comment, `atlas explain`, the order of work, co-change, untested, unread and look-alike sections, and the engine hardened on a Python and a TypeScript repository it had never seen]; [release v1.13.0 cut 2026-09-23 — Atlas ships as the eighth package and the seventh published one: the page written from the repository alone (doors, reach, landing places, readers, origin), the site page with the flow picture, `atlas check` in CI, the ladder and the three profile renders removed; the specification of record is `docs/atlas-page.spec.md`]; [release v1.12.0 cut 2026-09-06 — the armature run's harness fixes: `adjudicate --wave`, `SWARM_VERIFY_STEP_TIMEOUT_MS`, cost bounds rendered into every parallel amend prompt, the jury runner sending its measured `num_ctx`, nocase closure globs, the roadmap recurrence scope; the executor seat re-ruled to Opus by default with Sonnet where the task suits it and Fable for complex work and the clerk brief; SECURITY contact moved to GitHub advisories and four operator-identity strings scrubbed]; [release v1.11.0 cut 2026-08-26 — dogfood-swarm 3 on run `swarm-1787700871-d537`: `coordinator` ownership class, `--isolate` as the dispatch default, `swarm doctor` environment checks, Phase 9 as a run status not a dispatchable phase, Set-1/Stage C/D health fixes; ledger 33 fixed / 4 deferred / 11 unverified / 0 CRIT / 0 HIGH; run stays at `test`]; [release v1.10.0 cut 2026-07-17 — the trajectory layer (compiled roadmap artifact + advisory attention list + bounded operator notes) and the evidence-bearing closure verbs (`swarm reopen`/`close`), then the wave-40→44 confirming-audit arc that made the pass's own closure machinery actually close it: file-less routing/vouching/fingerprint-fusion + honor-reuse fixes, the Amendment 3 roadmap-artifact vocabulary reconciliation (first artifact to validate against the full schema), the T4 seeding flags built end to end, and the explicit-seed / UTC / phases-consolidation fixes. Ledger at release: 473 fixed, 18 deferred, 16 rejected, zero open; run swarm-1784091637-5127 stays OPEN (a waypoint, not closure)]; [release v1.9.0 cut 2026-07-03 — the full-dogfood-swarm release: four-stage health pass (~162 pinned fixes, run swarm-1783007856-9fdb) + approved feature pass — `swarm defer`/`reject` verbs, `dogfood-verify lint --scenario`, required-steps enforcement wired end-to-end (404 → accepted-with-warning), self-dogfood workflow + badge, read-model handbook page, revalidate full-coverage reclassification; operational faults (PROVENANCE_FAULT / VALIDATOR_FAULT_* / scenario-fetch-fault) now throw instead of persisting `_rejected`; findings package test discovery un-orphaned 16 files; suite ~2,106 → ~2,700+ tests]; [release v1.8.0 cut 2026-06-30 — VERIFY-F3: author-time `policy-lint` verb (`dogfood-verify lint <policy-file>`) — the `opa check` analogue: structural gate + data-independent predicate checks (`unknown_field`/`max_depth`/`node_budget`) over every `when` with no submission, plus the advisory `[]`-footgun warning (negation-parity suppression, cross-family-jury-improved) + honest static-coverage boundary; wired into `npm run verify` via `scripts/lint-policies.test.mjs` (no 5th workflow)]; [release v1.7.0 cut 2026-06-30 — VERIFY-F1: declarative no-eval policy-predicate engine (field/op/value + all/any/not/implies authored in YAML), `attested-if-human` migrated as the byte-identical differential-equivalence proof, DoS budgets + structural non-weakening + the `policy-config:` diagnostic class; cross-family-jury-hardened (DoS + `[]` fail-open + origin-misclassification)]; [release v1.6.0 cut 2026-06-29 — dogfood swarm: health pass A–D (41 findings) + honesty/capability feature pass: forbidden_tags policy gate made real, served shields.io badges + trends, `swarm clean` verb, `dogfood-report --status` consumer confirm, accepted-with-warning channel, `--json`/`--dry-run`/`--grep` ergonomics]; [release v1.5.0 cut 2026-06-21 — dogfood swarm: health hardening + record integrity (hash chain + opt-in XRPL anchor) + GitLab provenance + consumer onboarding]; [release v1.4.0 cut 2026-06-13 — health pass + 4-wave feature pass]; [release v1.3.1 cut 2026-06-01](https://github.com/dogfood-lab/testing-os/releases/tag/v1.3.1); release v1.3.0 cut 2026-06-01; release v1.2.3 cut 2026-05-20; first stable v1.0.0 cut 2026-04-25). Seven of eight `@dogfood-lab/*` packages publish on a release tag (every package not marked `private`); `portfolio` remains workspace-internal. `atlas` joins the published set at the first tag after 2026-09-22. The README's `<!-- version:start -->` block is auto-stamped by `scripts/sync-version.mjs` (runs as `prebuild`). Use `npm run sync-version:check` as a CI gate when you bump.

### TypeScript
`tsconfig.base.json` is the only place to set compiler options. Per-package `tsconfig.json` extends it and adds `outDir`/`rootDir`/`include`. `composite: true` everywhere. Never set `baseUrl` (deprecated under TypeScript 6). The TS5101 that bit repo-knowledge in CI came from tsup 8.5.1 injecting `baseUrl` into its own DTS pass, not from the tsconfig; the fix there was `ignoreDeprecations: "6.0"` in `tsup.config.ts` `dts.compilerOptions`, scoped to that pass.
...
### Ship gate
`SHIP_GATE.md` at the repo root tracks what shipcheck audits. Hard gates A–D (Security, Errors, Operator Docs, Hygiene) currently pass at 100% (22 checked / 15 SKIP-with-justification / 0 unchecked at v1.23.0, re-affirmed 2026-09-26 against the current SHIP_GATE.md). Soft gate E (Identity) is fully met. Re-run `npx @mcptoolshop/shipcheck audit` before any release; if a previously-checked item fails, fix the underlying gap before bumping the version.
