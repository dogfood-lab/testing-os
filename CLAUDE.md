...
### Ship gate
`SHIP_GATE.md` at the repo root tracks what shipcheck audits. Hard gates A–D (Security, Errors, Operator Docs, Hygiene) currently pass at 100% (22 checked / 15 SKIP-with-justification / 0 unchecked at v1.14.0, re-affirmed 2026-09-23 against the current SHIP_GATE.md). Soft gate E (Identity) is fully met. Re-run `npx @mcptoolshop/shipcheck audit` before any release; if a previously-checked item fails, fix the underlying gap before bumping the version.
