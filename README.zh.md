...
<!-- version:start -->
**v1.17.0** — 当前版本。有关已发布内容，请参阅 [CHANGELOG.md](CHANGELOG.md)。
<!-- version:end -->
...
- **您可以控制的策略契约。** 在 YAML 中声明什么被认为是“已验证”——一个有界的、无评估谓词 DSL（`field`/`op`/`value` + `all`/`any`/`not`/`implies`）——并在整个仓库中强制执行。在发布之前使用 `dogfood-verify lint` 检查策略。
- **一个并行代理群协议。** 针对代码库运行多代理审计，然后将原始结果转换为可重用的模式和原则。
- **一个实时状态界面。** 每个仓库的记录、索引和一个状态徽章，所有这些都来自一个证据存储。
- **一个页面，说明一个仓库是如何工作的。** Atlas 读取仓库的工作流程和清单、它们运行的工具、其导入、写入和历史记录，并编写 `atlas/README.md`：输入的内容、运行的内容、最终结果、谁阅读它、什么导致了什么问题、自上次映射以来发生了什么变化、从哪里开始。其中的任何句子都不是由人编写的；`atlas check` 在映射不再与代码匹配时，CI 会失败，`atlas explain <file>` 会回答一个文件在系统中的位置，并且每个拉取请求都会获得结构差异作为注释。

它是 [Dogfood Lab](https://github.com/dogfood-lab) 组织的旗舰单仓库项目——八个 `@dogfood-lab/*` 包，使用一个 `swarm` CLI 和一个 `atlas` CLI。
...
公共站点会呈现所有采用 Atlas 的公共仓库。对于必须保留在您机器上的仓库，相同的引擎以容器形式提供，并具有持久内存：
...
testing-os 处理通过 `repository_dispatch` 从 `mcp-tool-shop-org/*` 和 `dogfood-lab/*` 下的受信任 GitHub 仓库发送的 dogfood 提交。验证器需要 CI 来源证明——声明的运行 ID 通过提供商的 API 进行确认，并且具有格式错误的形状、缺少引用或无效策略声明的提交将被拒绝。

**来源证明是证明。** 对于 `github` 提交，验证器会确认声明的 GitHub Actions 运行确实存在（GitHub API），并将提交的 `repo` 和 `commit_sha` 绑定到该已确认的运行——这是一种实时的、无需密钥的检查，其根源在于 GitHub 自己的 OIDC 身份，因此记录不能证明不存在的运行或提交。**GitLab CI** 默认支持（`source.provider: gitlab`）；GitLab 提交是验证器调用非 GitHub 主机（`gitlab.com/api`）的唯一情况，并且仅适用于 `gitlab` 提交。

**记录完整性是防篡改的，但不是完全防篡改的。** 每个持久化的记录都携带一个 `integrity` 块（`submission_digest` + `prev_digest`），形成一个仅追加的哈希链，该链由 `node packages/ingest/run.js --verify-chain` 完全离线验证——检测外部篡改、磁盘损坏和部分恢复。它**不**防御对摄取凭据本身的攻击，该凭据可以重写记录和链；要解决这个问题，需要一个超出编写者控制范围的锚点。一个**可选的、默认关闭的 XRPL 锚点**（`node packages/ingest/run.js --anchor-*`）将链头记录到公共 XRP Ledger，从而可以检测任何低于已锚定点的截断或重写——这是第二个公开的非 GitHub 调用，并且仅当操作员启用它时才会发生。

**testing-os 涉及的内容：**每个 `repository_dispatch` 负载中的提交 JSON；此仓库中的 `policies/`、`fixtures/`、`records/`、`indexes/` 和 `dogfood/roadmap/`（最后一个仅由操作员调用的 `swarm roadmap compile` 写入——绝不会通过自动化的引入流程写入）；对 `api.github.com` 的外部调用，用于验证溯源；以及——仅针对 `github` 提交——对提交仓库的 `dogfood/scenarios/<scenario_id>.yaml` 的只读读取，读取的是已验证的提交（场景定义，用于支持必需步骤的强制执行；在使用前会限制大小并验证模式，如果缺少文件，则该检查将不会执行，并显示可见的警告）。

**testing-os 不涉及的内容：**超出已声明的 `dogfood/scenarios/` 定义文件的消费者源代码、消费者仓库中的密钥（超出分发信封范围），或此仓库工作树之外的任何内容。

**发现状态转换具有证据意义且仅追加。**群集控制平面的关闭动词（`swarm reopen`、`swarm close`）需要明确的原因、证据，以及——对于操作员关闭——声明的验证模式；每次转换都会写入一个不可变的 `finding_events` 行，记录执行操作的权限。没有自动化的流程可以根据过时情况关闭发现，也没有自动化的流程可以根据预测重新打开发现，也没有任何动词可以重写事件历史——错误使用的凭据可以添加转换，但每次添加本身都会被记录。

**网络接口。**默认情况下，唯一的出站连接是 `api.github.com`（只读：溯源确认 + 上述场景定义读取）。两个例外都是可选的，并且已在上面说明：GitLab 提供商提交（`gitlab.com/api`）和操作员启用的 XRPL 锚点运行。**没有遥测数据，没有分析数据——此代码库绝不会主动连接到外部服务器；如果没有这两个可选路径，它不会暴露任何超出 GitHub 之外的网络接口。**接收器工作流程的范围仅限于此仓库中的 `contents: write`。
