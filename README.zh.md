...
<!-- version:start -->
**v1.18.0** — 当前版本。有关已发布内容，请参阅 [CHANGELOG.md](CHANGELOG.md)。
<!-- version:end -->
...
- **您可以控制的策略合同。** 在 YAML 中声明什么被认为是“已验证”——一个有界的、无评估谓词 DSL（`field`/`op`/`value` + `all`/`any`/`not`/`implies`）——并在您的仓库中强制执行。在发布之前使用 `dogfood-verify lint` 检查策略。
- **一个并行代理群协议。** 针对代码库运行多代理审计，然后将原始结果转换为可重用的模式和原则。
- **一个实时状态界面。** 每个仓库的记录、索引和一个状态徽章，所有这些都来自一个证据存储。
- **一个页面，说明一个仓库是如何工作的。** Atlas 读取仓库的工作流程和清单、它们运行的工具、其导入、写入和历史记录，并编写 `atlas/README.md`：输入的内容、运行的内容、最终结果、谁阅读它、什么导致什么出错、自上次映射以来发生了什么变化、从哪里开始。其中的任何句子都不是由人编写的；`atlas check` 在映射不再与代码匹配时，CI 会失败；`atlas explain <file>` 回答一个文件在系统中的位置，并且每个拉取请求都会获得结构差异作为评论。
...
想要将您自己的仓库的测试证据记录到这里吗？**[`examples/` 启动工具包](examples/)** 可以在五分钟内让您开始（`dogfood-report` 构建提交；`dogfood-init` 搭建工作流程）。操作员指南、CLI 参考、模式参考和集成配方都位于 **[手册](https://dogfood-lab.github.io/testing-os/handbook/)** 中。每个版本的详细信息都位于 [CHANGELOG.md](CHANGELOG.md) 中。

## 在私有环境中运行

公共站点会呈现每个采用 Atlas 的公共仓库。对于必须保留在您机器中的仓库，相同的引擎以容器形式提供，并具有持久内存：
...
**来源证明是证明。** 对于 `github` 提交，验证器会确认声明的 GitHub Actions 运行确实存在（GitHub API），并将提交的 `repo` 和 `commit_sha` 绑定到该已确认的运行——这是一种实时的、无密钥的检查，其根源在于 GitHub 自己的 OIDC 身份，因此记录不能证明不存在的运行或提交。**GitLab CI** 默认支持（`source.provider: gitlab`）；GitLab 提交是验证器调用非 GitHub 主机（`gitlab.com/api`）的唯一情况，并且仅用于 `gitlab` 提交。

**记录完整性是防篡改的，但不是完全防篡改的。** 每个持久化的记录都携带一个 `integrity` 块（`submission_digest` + `prev_digest`），形成一个仅追加的哈希链，该链由 `node packages/ingest/run.js --verify-chain` 完全离线验证——检测非预期篡改、磁盘损坏和部分恢复。它**不**防御对注入凭据本身的攻击，该凭据可以重写记录和链；要解决这个问题，需要一个在写入者控制之外的锚点。一个**可选的、默认关闭的 XRPL 锚点**（`node packages/ingest/run.js --anchor-*`）将链头记录到公共 XRP Ledger，从而可以检测任何低于已锚定点的截断或重写——这是第二个公开的非 GitHub 调用，并且仅当操作员启用它时才会发生。

**testing-os 涉及的内容：**每个 `repository_dispatch` 负载中的提交 JSON；此仓库中的 `policies/`、`fixtures/`、`records/`、`indexes/` 和 `dogfood/roadmap/`（最后一个仅由操作员发起的 `swarm roadmap compile` 写入——绝不会由自动化的数据引入流程写入）；向 `api.github.com` 发起的外部调用，用于验证溯源；以及——仅针对 `github` 提交——对提交仓库的 `dogfood/scenarios/<scenario_id>.yaml` 的只读读取，读取的是经过验证的提交（场景定义，用于支持必需步骤的强制执行；在使用前会限制大小并验证模式，如果缺少文件，则该检查将不会被强制执行，并会显示可见的警告）。

**testing-os 不涉及的内容：**超出已声明的 `dogfood/scenarios/` 定义文件的消费者源代码、消费者仓库中超出分发信封的密钥，或此仓库工作树之外的任何内容。

**查找状态转换具有证据意义且仅追加。**集群控制平面的关闭动词（`swarm reopen`、`swarm close`）需要明确的原因、证据，以及——对于操作员关闭——声明的验证模式；每次转换都会写入一个不可变的 `finding_events` 行，记录执行操作的授权机构。没有自动化的流程可以基于过时状态关闭一个查找结果，也没有自动化的流程可以基于预测重新打开一个查找结果，也没有任何动词可以重写事件历史——错误使用的凭据可以添加转换，但每次添加本身都会被记录。

**网络接口。**默认情况下，唯一的出站连接是 `api.github.com`（只读：溯源确认 + 上述的场景定义读取）。两种例外情况都是可选的，并且已在上面说明：一个 GitLab 提供商提交（`gitlab.com/api`），以及一个由操作员启用的 XRPL 锚点运行。**没有遥测数据，没有分析数据——此代码库绝不会主动连接到外部服务器；如果没有这两个可选路径，它不会暴露任何超出 GitHub 的网络接口。**接收器工作流程的范围仅限于此仓库中的 `contents: write`。
